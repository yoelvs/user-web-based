...
// Populate currency dropdowns for Edit Modal
fetch('https://v6.exchangerate-api.com/v6/28d80ac21c82ae0f18de0c89/latest/USD')
  ...
  .then(data => {
    if (data && data.conversion_rates) {
      ...
      const baseSelect = document.getElementById("editBaseCurrency");
      const targetSelect = document.getElementById("editTargetCurrency");
      ...
      // Re-select user's current values if they exist
      const email = localStorage.getItem("loggedInUser");
      const user = JSON.parse(localStorage.getItem(email));
      if (user) {
        baseSelect.value = user.baseCurrency || "";
        targetSelect.value = user.targetCurrency || "";
      }
    }
  })
